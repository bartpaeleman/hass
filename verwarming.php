...
$pageTitle = $configData['pages']['verwarming.php']['name'] ?? 'VERWARMING';

if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
$currentRole = $_SESSION['role_level'] ?? 10;
$allowedRoles = $configData['pages']['verwarming.php']['roles'] ?? [99, 50, 10, 0];
if ('verwarming.php' !== 'index.php' && !in_array($currentRole, $allowedRoles) && $currentRole < 99) {
    ...
    <div class="thermo-info" style="display:flex; flex-direction:column; justify-content:space-between; height:100%; width:100%;">
      ...
      <div class="thermo-stats-custom" style="display:grid; grid-template-columns: 1fr 1fr; gap:8px;">
        ...
        <div class="thermo-stat">
          ...
          ${(() => {
            const cs = getComfortStatus(currentTemp, room.name);
            return `<span class="thermo-stat-val ${cs.cls}" style="font-size: 13px; color: ${cs.color};">${cs.statusText}</span>`;
          })()}
        </div>
        <div class="thermo-stat">
